...
def Convert_data(dt_ext):
	...
	#separa o dia		
	dia = re.findall(',(.\d{1,2}\s)',dt_ext)[0].strip()
						...
						try: # elimina os blocos que não contém "lines" e consequentemente não tem textos
							...
							for x in range(len(lines)):
								...
								for u in spans:
									...
									# parte de coletar a data na primeira página
									if num_pag == 1 and tam == "9" and flag == "4":
										dt_diario = re.findall('disponibilização:(\s.*\.\s)',u['text'])[0]
							...
							if len(txt_block) > 0:
								...
								nomes_pastas.append(nome_pasta_ajus)
...
def Juntar_blocks(numeros_paginas,nome_doc, nomes_pastas, txt_unific,ano, num_arq):
	...
	df_textos_paginas.to_excel("Diarios_publicacoes_BA_"+str(nome_pst[0])+'_'+str(num_arq)+".xlsx", index = False)
