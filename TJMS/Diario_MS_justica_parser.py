...
def Separar_textos_paginas(ano):
	...
	for b in tqdm(range(len(pastas))):
		...
		for a in range(len(arquivos)):
			...
			Juntar_blocks(numeros_paginas,nome_doc, nomes_pastas, txt_unific,ano, cabecalhos,a)								
...
def Juntar_blocks(numeros_paginas,nome_doc, nomes_pastas, txt_unific,ano, cabecalhos,num_arq):
	...
	df_textos_paginas.to_excel("Diarios_publicacoes_MS_"+str(nome_pst[0])+'_'+str(num_arq)+".xlsx", index = False)
