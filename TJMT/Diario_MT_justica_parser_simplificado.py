...
def separacao_padroes(num_caract,txt):
	...
	for pub in publis:
		if re.search("XXXXX", pub):
			pedac = pub.split("XXXXX")
			# print(pedac)
			# z= input("")
			for ped in pedac:
				if len(ped)> 5:
					ped = ped.replace("XXXXX","").strip()	
					publis_l.append(ped)
		else:
			publis_l.append(pub)		
	# print("vários")
...
def Juntar_blocks(numeros_paginas,nome_doc, nomes_pastas,txt_unific,ano, num_arq):
	...
	for txt,num,doc,pst in zip(txt_unific,numeros_paginas,nome_doc,nomes_pastas):
		...
		if re.search('Protocolo:\s\d{3,6}\W\d{4}',txt):
		...
		elif re.search('N\. \d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}|Classe: CNJ-\d{3}\s|CLASSE CNJ|N(°|º) \d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d{3}\.\d{4}|intimação da parte autora\s\s|intimação das partes\s\s|intimação para advogado',txt, re.IGNORECASE):

			numer_2 = re.findall('N\. \d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}', txt)
			numer_3 = re.findall(r'Classe: CNJ-\d{3}\s|CLASSE CNJ', txt,re.IGNORECASE)
			numer_4 = re.findall('N(°|º) \d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d{3}\.\d{4}', txt, re.IGNORECASE)
			numer_5 = re.findall("intimação da parte Autora\s\s|intimação das partes\s\s|intimação para advogado", txt, re.IGNORECASE)

			numer_2 = numer_2 + numer_5
			numer_3 = numer_3 + numer_4
			...
			for item in numer_3:
				...
				if num_caracter not in num_caract_2:
					num_caract_2.append(num_caracter-40)

				# tratamento para o caso de termos dois números CNJ iguaus na mesma página, ele verifica a próxima posição
				else:
					num_caracter = txt.find(item,num_caract[-1]+28,len(txt)) # por default são 28, porque é maior que um número CNJ
					num_caract.append(num_caracter_2-40)
		...
		elif re.search('Protocolo Número/Ano:',txt):# or re.search(r'Classe: CNJ-\d{3}\s|Protocolo Número/Ano:',txt):
			list_numbers = re.findall('Protocolo Número/Ano:',txt)
			# if num == 22:
			# 	print(list_numbers)
			# 	print(txt)
			# 	z=input("")
		...
		for item in publis:
			inic_text = item[:90]
			# if num > 104:
			# 	if re.search("intimação da parte Autora\s\s|intimação das partes\s\s",inic_text,re.IGNORECASE):
			# 		print(num)
			# 		print(inic_text)
			# 		z= input("")
			...
			if re.search('Protocolo: \d{3,6}\W\d{4}|N\. \d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}|N(°|º) \d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d{3}\.\d{4}|Classe: CNJ-\d{3}\s|CLASSE CNJ|Protocolo Número/Ano:|intimação da parte autora\s\s|intimação das partes\s\s|intimação para advogado',inic_text, re.IGNORECASE): # pesquisa o padrão em todas as linhas da publicação (dentro do limite de caracteres)
				
				# salva nas listas a publicação e as demais informações dela (página, documento, pasta)	
				if len(publicacoes) > 0 and len(publicacoes[-1])< 89:
			...
			# caso ele não encontre o padrão CNJ e essa publicação não seja a primeira da lista 
				...
				if re.search("Portaria|Despachos|Edital de",txt[:40], re.IGNORECASE):
					pula = True
	...
	if len(publicacoes) == 0:
		...
		for n in range(len(publicacoes)):
			try:
				nm_proc = re.search('\d{3,6}\W\d{4}|\d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}|\d{3,6}\s*\W\s*\d{4}|\d{1,7}(?:-|.{2}).\d{2}\.\d{4}\.\d{3}\.\d{4}',publicacoes[n], re.IGNORECASE).group().replace(" ","") # se encontrar o padrão completo, separa o número
