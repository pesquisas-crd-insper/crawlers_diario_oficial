...
def Separar_textos_paginas(ano,estado):
	...
	diret = r'./Diarios_TRF1_'+estado+"_"+ano
...
def Juntar_blocks(numeros_paginas,nome_doc,nomes_pastas,txt_unific,ano,num_arq,estado):
	...
	if len(publicacoes) == 0:
		...
		if len(publicacoes_l) == 0:
			...
			df_textos_paginas["estado"] = str(estado)
			...
			path = dir_path + f'\Diarios_processados_TRF1_'+estado+'_csv_'+str(ano)
			Path(path).mkdir(parents=True, exist_ok=True)

			# gera o excel com o DF final

			df_textos_paginas.to_csv(path+"\Diarios_publicacoes_TRF1_"+estado+'_'+str(df_textos_paginas["nomes_pastas"][0])+"_"+str(num_arq)+".csv", index = False)
...
def main():
	...
	print("*"*20)
	print()
	print("Para cada ano será realizado o parseamento dos seguintes Estados:")
	print()
	print("TRF1, GO, AC, AM, AP, RR, DF, PA, BA, MG, TO, PI, MA, MT, RO")
	print()
	ano = input("Digite o ano com 4 dígitos (Ex:2012):")
	print()
	print("*"*20)
	estados = ["TRF1", "GO", "AC", "AM", "AP", "RR", "DF","PA","BA","MG","TO","PI","MA","MT","RO"]
	# estados = ["TRF"]
	if int(ano) > 2014:
		for estado in estados:
			...
			try:
				...
				print("provalvel que não tenhamos a combinação", ano,"e", estado)
	else:
		estado = "TRF"
		data_frames = Separar_textos_paginas(ano, estado)
